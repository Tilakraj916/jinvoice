package com.sergiopino.webapp.spring;
 ...
 * add validation configuration for their own persistent classes to an JInvoice application without modifying any of the
 * existing JInvoice Spring configuration or jar distribution files.

 * <p>As an example consider the following Spring bean configuration:

 * <pre>
 * &lt;bean class=&quot;com.sergiopino.spring.ValidatorExtensionPostProcessor&quot;&gt;
 ...
 * process coupled with JInvoice's configuration file auto detection will ensure that the validation extension is
 * automatically included into the application without requiring any modification of JInvoice's existing config files.
 *
